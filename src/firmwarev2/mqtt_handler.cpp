...
bool reconnect_mqtt() {
  ...
  if (ultima_tentativa_reconnect != 0 && (now - ultima_tentativa_reconnect) < 30000) {
    // Evita flood de tentativas muito rápidas (30s intervalo)
    return false;
  }
...
void newListaEquipamentos() {
  if (mqttClient.publish(mqtt_topic_req_eqps, "")) {
    Serial.println("[MQTT] Solicitada nova lista de equipamentos");
    Serial.print("Estado do cliente MQTT: ");
    Serial.println(mqttClient.state());
  } else {
    Serial.println("[MQTT] Erro ao solicitar nova lista de equipamentos");
    Serial.println(mqttClient.state());
  }
}
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, mqtt_topic_res_usrs) == 0) {
    ...
    if (error) {
      Serial.print("Erro JSON usuarios: ");
